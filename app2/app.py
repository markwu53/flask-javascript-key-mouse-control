...
work_dir = r"C:\temp"
screen_image = work_dir+r"\screen.jpg"
sqlite_db = r"screen.db"
vwidth = 65536
vheight = 65536
...
def save():
    ...
    sql_format = """
insert into actions 
(ActionTitle, ActionStep, ActionText, InsertTime)
values
('{title}', {step}, '{action}', datetime())
"""
    for step, action in enumerate(actions):
        sql = sql_format.format(title=title, step=step+1, action=action)
        cursor.execute(sql)
    conn.commit()
    conn.close()
    return ""
...
select ActionText
from actions
where ActionTitle = '{title}'
order by ActionStep
"""
    sql = sql_format.format(title=title)
    cursor.execute(sql)
    actions = []
    for row in cursor:
    ...
    for action in actions:
        ...
        if type == "mouse":
            ...
            if subtype == "move":
                x, y = splits[2:]
                x = int(float(x) * vwidth)
                y = int(float(y) * vheight)
                MoveMouse(x, y)
                time.sleep(.1)
            elif subtype == "left":
                LeftClick()
